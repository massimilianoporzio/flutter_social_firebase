...
void main() {
  test('empty Auth User has correct default values', () {
    ...
    expect(AuthUser.empty.photURL, equals(''));
  });

  test('Two Auth User with same values are equal', () {
    const user1 =
        AuthUser(id: "id", email: "email", name: "name", photURL: 'photoURL');
  ...
  test('Two Auth User with different id values are NOT equal', () {
    const user1 =
        AuthUser(id: "id1", email: "email", name: "name", photURL: 'photoURL');

    const user2 =
        AuthUser(id: "id2", email: "email", name: "name", photURL: 'photoURL');

    expect(user1, isNot(equals(user2)));
  });
  test('Two Auth User with different email values are NOT equal', () {
    const user1 =
        AuthUser(id: "id", email: "email1", name: "name", photURL: 'photoURL');

    const user2 =
        AuthUser(id: "id", email: "email2", name: "name", photURL: 'photoURL');

    expect(user1, isNot(equals(user2)));
  });
  test('Two Auth User with different name values are NOT equal', () {
    const user1 =
        AuthUser(id: "id", email: "email", name: "name1", photURL: 'photoURL');

    const user2 =
        AuthUser(id: "id", email: "email", name: "name2", photURL: 'photoURL');

    expect(user1, isNot(equals(user2)));
  });
  test('Two Auth User with different photoURL values are NOT equal', () {
    const user1 =
        AuthUser(id: "id", email: "email", name: "name", photURL: 'photoURL1');

    const user2 =
        AuthUser(id: "id", email: "email", name: "name", photURL: 'photoURL2');
